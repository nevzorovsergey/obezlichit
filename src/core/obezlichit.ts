...
export interface OutputFile { name: string; bytes: Uint8Array; report: RewriteReport }
...
export async function applyPlan(plan: Plan): Promise<OutputFile[]> {
  ...
  for (const f of plan.files) {
    ...
    for (const m of f.models) rewritePage(f.doc, m, [...pairs.entries()], report, hits);
    report.notFound = [...pairs.keys()].filter((k) => !hits.get(k));
    scrubMetadata(f.doc);
    out.push({ name: f.name, bytes: await f.doc.save({ useObjectStreams: false, updateFieldAppearances: false }), report });
  }
  return out;
}
