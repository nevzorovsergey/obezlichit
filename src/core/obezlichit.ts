...
import { coverOf, pageModel, rewritePage, type Fallback, type PageModel, type RewriteReport } from "./pdf/rewrite";
...
export interface Substitution {
  ...
  replacement: string;
  count: number;
  /** в шрифтах документа нет всех букв подмены — нужен запасной шрифт */
...
export async function makePlan(inputs: InputFile[], rng: Rng = cryptoRng()): Promise<Plan> {
  ...
  for (const f of files) for (const x of f.finds) {
    ...
    const replacement = synth.generate(x, cover);
    const needsFallback = !!cover && [...replacement].some((ch) => ch !== " " && !cover!.has(ch));
    substitutions.set(key, { key, kind: x.kind, original: x.value, replacement, count: 1, needsFallback });
  }
...
export async function applyPlan(plan: Plan): Promise<OutputFile[]> {
  ...
  for (const f of plan.files) {
    ...
    const keys = new Set(f.finds.map((x) => keyOf(x.kind, x.value)));
    const fallback: Fallback | undefined = [...plan.substitutions.values()].some((s) => s.needsFallback && keys.has(s.key)) ? await makeFallback(f.doc) : undefined;
    ...
    for (const m of models) rewritePage(f.doc, m, [...pairs.entries()], report, hits, fallback);
